...
| 時點 | 內容 | 機制 |
|---|---|---|
| 每刀 | 固定 **RPG 職業經驗**（config 可調；每日上限天生封頂 faucet） | 攻擊交易內發放 |
| 清輪 | 進度公告（「第 N 輪討伐成功」），**不發料** | 純顯示 |
| 賽季末 | 依本季總傷害排名：少量女神石 + 賽季稱號 | cron 冪等結算 |

- 每刀經驗發 **RPG 職業經驗**（`minigame_level`，非聊天經驗）：職業等級是傷害公式的輸入（`makeCharacter(job_key, level)`），世界王必須持續餵職業經驗，成長迴圈才成立。聊天經驗管線（chatXp pipeline）無外部發放 API 且曲線由 prestige 重構管理，不從世界王注入。固定值取代舊「傷害比例 × boss.exp」公式；等差懲罰隨之廢除；裝備 `exp_bonus` 仍加成。
