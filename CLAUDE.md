...
- Three services: **bot** (`yarn start`), **frontend** (pre-built static), **worker** (`yarn worker`). Plus a one-shot **redive-migrate** service on the host compose: bot and worker `depends_on` it with `service_completed_successfully`, so every `docker compose up` (including the deploy timer) runs `knex migrate:latest` with the new image before the new bot/worker start — no manual migrate step on deploy.
